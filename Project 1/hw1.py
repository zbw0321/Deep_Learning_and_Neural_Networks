...
def my_perceptron(x):
    ...
    Weight = tf.get_variable("weight",shape=[x],initializer = tf.ones_initializer())
    y = tf.reduce_sum(tf.multiply(i,Weight))
    out = my_relu(y)
...
def onelayer(X, Y, layersize=10):
    ...
    """
    w = tf.Variable(tf.random_normal([X.shape[1].value, layersize], stddev=0.1))
    b = tf.Variable(tf.zeros([layersize]) + 0.1 )
...
def twolayer(X, Y, hiddensize=30, outputsize=10):
    ...
    """
    #layer one
    w1 = tf.Variable(tf.random_normal([X.shape[1].value, hiddensize], stddev=0.1))
...
def convnet(X, Y, convlayer_sizes=[10, 10], \
    ...
    """
    #conv_layer1
    conv1 = tf.layers.conv2d(inputs=X, filters=convlayer_sizes[0], kernel_size = filter_shape,
                                padding="same",activation = tf.nn.relu)
    #conv_layer2
    conv2 = tf.layers.conv2d(inputs=conv1,filters=convlayer_sizes[1], kernel_size = filter_shape,
                                padding="same",activation = tf.nn.relu )
    #fully-connected
    #shape become 28*28*convlayer_sizes[1]
    conv2_shape = tf.reshape(conv2,[-1,28*28*convlayer_sizes[1]])
    #call onelayer function
    w, b, logits, preds, batch_xentropy, batch_loss = onelayer(conv2_shape, Y, outputsize)
